    ...
    <div class="content-body" scroll-store="hostsList">
        <table id="hosts_table" class="table table-sm table-hover table-bordered table-fixed-header">
            ...
            <tr>
                ...
                <th scope="col" style="width: 250px;"><span>@lang('admin/hubs.host_ADDRESS')</span></th>
            ...
            <tbody>
            @forelse($data as $row)
                <tr data-id="{{ $row->id }}" data-group="{{ $row->group }}" class="{{ $row->lost ? 'row-with-ow-lost' : '' }}">
                    <td>{{ $row->id }}</td>
                    <td>{{ $row->typName }}</td>
                    <td class="nowrap">{{ $row->address }}</td>
                    <td>{{ $row->comm }}</td>
                    <td>{{ $row->channels }}</td>
                    <td>
    ...
    <script>
        $(document).ready(() => {
            $('#hosts_table tbody tr').on('click', function (e) {
                ...
                hostEdit($(this).data('group'), $(this).data('id'));
            });
        });

        function hostAdd() {
            dialog('{{ route("admin.hub-host-edit", ["hubID" => $hubID, "group" => 'none', "id" => -1]) }}');
        }

        function hostEdit(group, id) {
            dialog('{{ route("admin.hub-host-edit", ["hubID" => $hubID, "group" => "", "id" => ""]) }}/' + group + '/' + id);
        }
